...
export default function Index({ data, filters }: TimeSlotsIndexProps) {
    ...
    const handleDelete = () => {
        if (!deleteId) return;
        setIsDeleting(true);
        router.delete(timeSlotsDestroy(deleteId), {
            onFinish: () => {
                setIsDeleting(false);
                setDeleteId(null);
            },
    ...
    return (
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4 lg:p-6">
                <div className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
                    ...
                    <TablePageHeader
                        ...
                        createHref={timeSlotsCreate().url}
                        createLabel="New Time Slot"
                    />
                            ...
                            {data.data.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={5} className="h-40 text-center">
                                        <div className="flex flex-col items-center gap-3 text-muted-foreground">
                                            ...
                                            <div>
                                                <p className="text-sm font-medium">No time slots found</p>
                                                {search && (
                                ...
                                data.data.map((item) => (
                                    <TableRow
                                        ...
                                        onClick={() => router.get(timeSlotsShow(item.id))}
                                    >
                                        ...
                                        >
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end" className="w-40">
                                                    <DropdownMenuItem onClick={() => router.get(timeSlotsShow(item.id))}>
                                                        <Eye className="mr-2 h-4 w-4" />
                                                        View
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem onClick={() => router.get(timeSlotsEdit(item.id))}>
            ...
            <ConfirmDeleteDialog
                ...
                itemName={data.data.find((s) => s.id === deleteId)?.name}
...
Index.layout = (page: React.ReactNode) => (
    <AppLayout breadcrumbs={[{ title: 'Time Slots', href: timeSlots() }]}>
        {page}
    </AppLayout>
);

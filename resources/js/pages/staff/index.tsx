...
import type { StaffIndexProps } from '@/types';
import AppLayout from '@/layouts/app-layout';

export default function Index({ data, filters }: StaffIndexProps) {
    ...
    const handleDelete = () => {
        if (!deleteId) return;
        setIsDeleting(true);
        router.delete(staffDestroy(deleteId), {
            onFinish: () => {
                setIsDeleting(false);
                setDeleteId(null);
            },
                            ...
                            {data.data.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={4} className="h-40 text-center">
                                        <div className="flex flex-col items-center gap-3 text-muted-foreground">
                                            ...
                                            <div>
                                                <p className="text-sm font-medium">No faculty staff found</p>
                                                {search && (
                                ...
                                data.data.map((item) => (
                                    <TableRow
                                        ...
                                        onClick={() => router.get(staffShow(item.id))}
                                    >
                                        ...
                                        >
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end" className="w-40">
                                                    <DropdownMenuItem onClick={() => router.get(staffShow(item.id))}>
                                                        <Eye className="mr-2 h-4 w-4" />
                                                        View
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem onClick={() => router.get(staffEdit(item.id))}>
...
Index.layout = (page: React.ReactNode) => (
    <AppLayout breadcrumbs={[{ title: 'Faculty Staff', href: staff() }]}>
        {page}
    </AppLayout>
);

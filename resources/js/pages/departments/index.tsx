...
import { MoreVertical, Pencil, Trash2, Eye, Building2 } from 'lucide-react';
...
import type { DepartmentsIndexProps } from '@/types';
import AppLayout from '@/layouts/app-layout';

export default function Index({ data, filters }: DepartmentsIndexProps) {
    ...
    const handleDelete = () => {
        if (!deleteId) return;
        setIsDeleting(true);
        router.delete(departmentsDestroy(deleteId), {
            onFinish: () => {
                setIsDeleting(false);
                setDeleteId(null);
            },
    ...
    return (
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4 lg:p-6">
                <div className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
                    ...
                    <TablePageHeader
                        ...
                        createHref={departmentsCreate().url}
                            ...
                            {data.data.length === 0 ? (
                                ...
                                data.data.map((item) => (
                                    <TableRow
                                        ...
                                        onClick={() => router.get(departmentsShow(item.id))}
                                    >
                                        ...
                                        >
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end" className="w-40">
                                                    <DropdownMenuItem onClick={() => router.get(departmentsShow(item.id))}>
                                                        <Eye className="mr-2 h-4 w-4" />
                                                        View
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem onClick={() => router.get(departmentsEdit(item.id))}>
            ...
            <ConfirmDeleteDialog
                ...
                itemName={data.data.find((d) => d.id === deleteId)?.name}
